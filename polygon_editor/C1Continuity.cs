    ...
    class C1Continuity : Relation
    {
        ...
        public override void UpdateRelation(Vertex movingVertex, Stack<(Relation, Vertex)> relationsStack)
        {
            ...
            if (movingVertex == Vertex1)
            {
                ...
                otherVertex.X = controlPoint.X - 4 * dx;
                otherVertex.Y = controlPoint.Y - 4 * dy;
            }
            else
            {
                int dx = controlPoint.X - movingVertex.X;
                int dy = controlPoint.Y - movingVertex.Y;
                otherVertex.X = controlPoint.X - dx / 4;
                otherVertex.Y = controlPoint.Y - dy / 4;
            }
        ...
        private bool IsC1()
        {
            ...
            {
                if ((Vertex1.ControlPointPrev.X - Vertex1.X) * 3 / (Vertex1.X - Vertex2.X) - 1 < 1e-3 &&
                    (Vertex1.ControlPointPrev.Y - Vertex1.Y) * 3 / (Vertex1.Y - Vertex2.Y) - 1 < 1e-3)
                    return true;
                else
                    return false;
            }
